...
func (r *ReportRepositoryImpl) ReportGetById(ctx context.Context, id int) (*model.Report, error) {
	query := `
        SELECT id, month_of, worker_name, area_of_assignment, name_of_church,
               worship_service, sunday_school, prayer_meetings, bible_studies,
               mens_fellowships, womens_fellowships, youth_fellowships,
               child_fellowships, outreach, training_or_seminars,
               leadership_conferences, leadership_training, others, family_days,
               tithes_and_offerings, home_visited, bible_study_or_group_led,
               sermon_or_message_preached, person_newly_contacted, person_followed_up,
               person_led_to_christ, names, narrative_report,
               challenges_and_problem_encountered, prayer_request
        FROM reports
        WHERE id = $1
    `
	...
	err := r.Db.QueryRowContext(ctx, query, id).Scan(
		...
		&report.AreaOfAssignment,
		&report.NameOfChurch,
		&report.WorshipService,
		&report.SundaySchool,
		&report.PrayerMeetings,
		&report.BibleStudies,
		&report.MensFellowships,
		&report.WomensFellowships,
		&report.YouthFellowships,
		&report.ChildFellowships,
		&report.Outreach,
		&report.TrainingOrSeminars,
		&report.LeadershipConferences,
		&report.LeadershipTraining,
		&report.Others,
		&report.FamilyDays,
		&report.TithesAndOfferings,
		&report.HomeVisited,
		&report.BibleStudyOrGroupLed,
		&report.SermonOrMessagePreached,
		&report.PersonNewlyContacted,
		&report.PersonFollowedUp,
		&report.PersonLedToChrist,
		&report.Names,
		&report.NarrativeReport,
		&report.ChallengesAndProblemEncountered,
		&report.PrayerRequest,
